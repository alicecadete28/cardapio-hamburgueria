export function ModalCart({ totalCount, handleRemoveFromCart, carrinho }) {
  return(
    <div 
      ...
      >
        <div className='bg-white p-5 rounded-md min-w-[90%] md:min-w-[600px]'>
            ...
            <ul>
              {carrinho.map((item) => 
                <li key={item.id} className='flex items-center justify-between border-b-2 py-2'>
                  ...
                  <button className='bg-red-500 text-white rounded ml-2 px-2'
          ...
          <p className='font-bold'>Total: <span>{totalCount}</span></p>
          ...
          <div className='flex items-center justify-between mt-4 w-full'>
            ...
            <button className='bg-green-500 text-white py-1 rounded px-4' id='checkout-btn'>
              Finalizar pedido
            </button>
          </div>
        </div>
